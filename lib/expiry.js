...
//
// A job whose pickup window ended more than half an hour ago and which is still OPEN
// has, in practice, been abandoned: the courier who took it would arrive long
// after it was wanted. It is cancelled and refunded automatically (an unpaid one
// simply becomes "not charged"), the same as if the customer had cancelled it.
//
...
async function expireStaleJobs() {
  ...
  try {
    ...
    const stale = await sql`
      ...
      WHERE status = 'OPEN' AND pickup_window_end < ${cutoff}
