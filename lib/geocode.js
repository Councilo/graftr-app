...
// address fragment -> up to `limit` candidate matches, for the address
// autocomplete dropdown — the "pick from a list" every mainstream map app
// offers instead of asking someone to type a full address and hope it
// resolves. Same UK scoping and the same underlying endpoint as geocode()
// above, just more results and no picking-the-first-one for the caller.
async function searchAddresses(query, limit = 6) {
  const rows = await nominatimFetch(
    `/search?format=jsonv2&limit=${limit}&countrycodes=gb&q=${encodeURIComponent(query)}`,
  );
  if (!Array.isArray(rows)) return [];
  return rows.map((r) => ({ display_name: r.display_name, lat: Number(r.lat), lng: Number(r.lon) }));
}
...
module.exports = {
  geocode, searchAddresses, reverseGeocode, haversineKm, quotePrice, drivingRoute, computeQuote,
