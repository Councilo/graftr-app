...
const SCHEMA_VERSION = '2026-09-22-d';
...
async function buildSchema() {
  ...
  // ---- walkers, phase 2: partner shops ----
  // A shop is a customer account with one of these attached (owner_user_id), not a new role — the
  // owner still signs in and is treated as an ordinary customer everywhere except that they can also
  // post a bag. 'pending' until an admin approves it; only an approved shop can post one.
  await sql`
  ...
  `;
  // One shop per account: applying again edits the same row rather than creating a second one.
  await sql`CREATE UNIQUE INDEX IF NOT EXISTS shops_owner_idx ON shops (owner_user_id)`;

  // A shop-posted job is always a walker job (delivery_mode = 'walker'): the pickup is the shop's own
  // registered address, and pickup_code is what the shop reads out to prove the right walker is
  // stood in front of them — checked in handlers/jobs-pickup.js, the pickup-end equivalent of
  // delivery_pin at the other end. shop_ready_at / shop_handed_over_at are the shop's own two taps
  // ("the bag is packed" / "I gave it to them") — informational for the shop's own screen, neither
  // one gates the job's real status, which is still driven by accept/pickup/deliver as for any job.
  await sql`ALTER TABLE jobs ADD COLUMN IF NOT EXISTS shop_id INTEGER REFERENCES shops(id)`;
  await sql`ALTER TABLE jobs ADD COLUMN IF NOT EXISTS pickup_code TEXT`;
  await sql`ALTER TABLE jobs ADD COLUMN IF NOT EXISTS shop_ready_at TIMESTAMPTZ`;
  await sql`ALTER TABLE jobs ADD COLUMN IF NOT EXISTS shop_handed_over_at TIMESTAMPTZ`;
  await sql`CREATE INDEX IF NOT EXISTS jobs_shop_idx ON jobs (shop_id)`;
  ...
  // ---- walkers, phase 3: route mode ----
  // A walker can declare their walking route; the marketplace then filters and ranks jobs
  // by how much detour they would add. One row per walker: walker_id carries its own UNIQUE
  // constraint (declared inline, not as a separate CREATE UNIQUE INDEX — pg-mem's ON CONFLICT
  // resolution only recognises a conflict target that came from an inline PRIMARY KEY or UNIQUE
  // constraint, the same way schema_version's own upsert above relies on id PRIMARY KEY; a plain
  // CREATE UNIQUE INDEX enforces uniqueness fine but ON CONFLICT (walker_id) can't find it,
  // failing with "no unique or exclusion constraint matching the ON CONFLICT specification").
  // It is upserted each time a walker sets a new route and deleted when they clear it.
  await sql`
    CREATE TABLE IF NOT EXISTS walker_routes (
      id SERIAL PRIMARY KEY,
      walker_id INTEGER NOT NULL UNIQUE REFERENCES users(id) ON DELETE CASCADE,
      ...
      expires_at TIMESTAMPTZ NOT NULL,
      max_detour_m INTEGER NOT NULL DEFAULT 400,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `;
  await sql`ALTER TABLE walker_routes ADD COLUMN IF NOT EXISTS max_detour_m INTEGER NOT NULL DEFAULT 400`;
}
