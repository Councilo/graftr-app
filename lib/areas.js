...
function publicJobView(job) {
  return {
    ...
    status: job.status,
    masked: true,
    pickup_address: courierAreaLabel(job.pickup_address, 'Pickup point'),
    dropoff_address: courierAreaLabel(job.dropoff_address, 'Drop-off point'),
    pickup_lat: roundPin(job.pickup_lat),
    pickup_lng: roundPin(job.pickup_lng),
    ...
    // No stored route: the stored one runs to the exact doors. The app draws the real road route
    // between the two rounded pins instead (the same way it does while a quote is being made).
    route_geometry: null,
