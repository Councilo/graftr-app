...
// Never a random key invented at startup. A serverless function has no single
// long-running process: it cold-starts repeatedly, on however many instances load
// balances it, and a key made up fresh each time would invalidate every token
// constantly, for real users. The key is JWT_SECRET, or (see lib/secret.js) one
// worked out from the database connection string, which is the same everywhere.
// With neither there is nothing safe to sign with, so it fails loudly instead.
function requireSecret() {
  const secret = signingSecret();
  if (!secret) {
    const err = new Error('JWT_SECRET is not set');
    err.statusCode = 500;
    err.publicMessage = 'Server is not configured yet (missing JWT_SECRET and no database connected) — nothing you did wrong.';
    throw err;
  }
  return secret;
}
...
async function userFromRequest(req) {
  ...
  const { rows } = await sql`
    SELECT id, email, full_name, role, is_suspended, suspended_reason, deleted_at,
           terms_accepted_at, terms_version, location_consent_at, phone, email_verified_at, password_changed_at
  ...
  // A password reset signs out everything issued before it: a stolen session should not survive one.
  const { password_changed_at: changedAt, ...safe } = user;
  if (changedAt && payload.iat < Math.floor(new Date(changedAt).getTime() / 1000)) return null;
  return { ...safe, is_admin: isAdminEmail(user.email) };
}
