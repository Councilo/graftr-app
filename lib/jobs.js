...
function serializeJob(row, { forCustomer = false } = {}) {
  ...
  // The reverse, at the other end of a shop job: whether a pickup code is needed is the walker's
  // business (so their app can prompt for one), but the code itself is the shop's to read out —
  // if a walker's own app showed it to them, they'd never actually need to go and be told it,
  // which is the entire point of it. Stripped below along with delivery_pin.
  job.pickup_code_required = !!row.pickup_code;
