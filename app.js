...
function shopsForLocation() {
  ...
  const loc = locationChosen();
  if (loc) {
    list = list.filter(s => String(s.town || '').toLowerCase().includes(loc.toLowerCase()));
  }
  const f = shopFilter();
  if (f === 'clothes') return list.filter(s => s.category === 'Clothes');
  if (f === 'antiques') return list.filter(s => s.category === 'Antiques');
  if (f === 'icecream') return list.filter(s => s.category === 'Ice Cream');
  if (f === 'cafes') return list.filter(s => s.category === 'Cafes');
  if (f === 'bakery') return list.filter(s => s.category === 'Bakery & Delis');
  if (f === 'books') return list.filter(s => s.category === 'Books');
  if (f === 'florists') return list.filter(s => s.category === 'Florists');
  return list;
}
...
function shopCardHtml(shop) {
  ...
  const catBadge = shop.category ? `<span style="position:absolute;top:8px;left:8px;background:rgba(20,20,20,0.75);color:#fff;font-size:10px;font-weight:700;padding:3px 8px;border-radius:10px;backdrop-filter:blur(4px);z-index:2">${escapeHtml(shop.category)}</span>` : '';
  return `
    <div class="shop-card press biz-card" data-action="requestFromShop" data-arg="${arg}">
      <div class="biz-card-photo"${shop.photo ? ` style="background-image:url('${escapeHtml(shop.photo)}');position:relative"` : ''}>
        ${catBadge}
        <span class="biz-card-logo is-initials">${escapeHtml(initials)}</span>
      </div>
      <div class="biz-card-body">
...
function searchableShops() {
  const loc = locationChosen();
  if (loc) {
    return SHOPS_100.filter(s => String(s.town || '').toLowerCase().includes(loc.toLowerCase()));
  }
  return SHOPS_100;
}
